...
namespace B2B.Areas.Supermarket.Controllers
{
    public class OrdersController : Controller
    {
        ...
        public ActionResult ManageOrders(int? flag)
        {
            ...
            ViewBag.Total = db.OrderHeader.Where(mu => mu.OrderByUserId == loginUser.Id && mu.Status!= orderStatusEnum.NeedApproval).Count();
            ViewBag.Pending = db.OrderHeader.Where(mu => mu.OrderByUserId == loginUser.Id && mu.Status == orderStatusEnum.Pening).Count();
            ViewBag.Inprocess = db.OrderHeader.Where(mu => mu.OrderByUserId == loginUser.Id && mu.Status == orderStatusEnum.InProcess).Count();
            ViewBag.Enroute = db.OrderHeader.Where(mu => mu.OrderByUserId == loginUser.Id && mu.Status == orderStatusEnum.Enroute).Count();
            ViewBag.Delivered = db.OrderHeader.Where(mu => mu.OrderByUserId == loginUser.Id && mu.Status == orderStatusEnum.Delivered).Count();
            ViewBag.Cancelled = db.OrderHeader.Where(mu => mu.OrderByUserId == loginUser.Id && mu.Status == orderStatusEnum.Canceled).Count();
            ViewBag.Rejected = db.OrderHeader.Where(mu => mu.OrderByUserId == loginUser.Id && mu.Status == orderStatusEnum.Rejected).Count();
            
            List<OrdersVM> model = new List<OrdersVM>();
            if (flag == null)
            {
                model = db.OrderHeader.Where(mu => mu.OrderByUserId == loginUser.Id && mu.Status != orderStatusEnum.NeedApproval).Select(u => new OrdersVM
                {
                    Status = u.Status,
            ...
            {
                model = db.OrderHeader.Where(mu => mu.OrderByUserId == loginUser.Id && mu.Status == orderStatusEnum.Pening).Select(u => new OrdersVM
                {
                    Status = u.Status,
                    Date = u.CreatedOn,
                    ShippingCharges = u.ShippingCharges,
            ...
            {
                model = db.OrderHeader.Where(mu => mu.OrderByUserId == loginUser.Id && mu.Status == orderStatusEnum.InProcess).Select(u => new OrdersVM
                {
                    Status = u.Status,
                    Date = u.CreatedOn,
                    ShippingCharges = u.ShippingCharges,
            ...
            {
                model = db.OrderHeader.Where(mu => mu.OrderByUserId == loginUser.Id && mu.Status == orderStatusEnum.Enroute).Select(u => new OrdersVM
                {
                    Status = u.Status,
                    ShippingCharges = u.ShippingCharges,
                    Date = u.CreatedOn,
            ...
            {
                model = db.OrderHeader.Where(mu => mu.OrderByUserId == loginUser.Id && mu.Status == orderStatusEnum.Canceled).Select(u => new OrdersVM
                {
                    Status = u.Status,
                    ShippingCharges = u.ShippingCharges,
                    PaymentStatus = u.PaymentStatus,
            ...
            {
                model = db.OrderHeader.Where(mu => mu.OrderByUserId == loginUser.Id && mu.Status == orderStatusEnum.Rejected).Select(u => new OrdersVM
                {
                    Status = u.Status,
                    ShippingCharges = u.ShippingCharges,
                    PaymentStatus = u.PaymentStatus,
        ...
        public ActionResult AuthOrders()
        {
            ...
            List<OrdersVM> model = db.OrderHeader.Where(mu => mu.OrderByUserId == loginUser.Id && mu.Status== orderStatusEnum.NeedApproval).Select(u => new OrdersVM
            {
                Status = u.Status,
                ShippingCharges = u.ShippingCharges,
                PaymentStatus = u.PaymentStatus,
        ...
        public ActionResult Add(int? categoryId,decimal? minPrice,decimal? maxPrice,string search)
        {
            ...
            if (categoryId == null && search == null && (minPrice==null || maxPrice==null))
            {
                var models = db.StoreProducts.Where(prd => prd.Status == UserStatus.Active && prd.InStock > 0).Select(u => new MarketProductDtos
                {
                    ...
                    Tax = u.Tax,
                    CompanyName=u.CompanyProxy.Name,
        ...
        public ActionResult SupervisorOrdersReport(DateTime? startDate,DateTime? endDate,string supervisorId)
        {
            var loginUser = getLoginUser();
            ...
            {
                List<OrdersVM> model = db.OrderHeader.Where(mu => mu.OrderCreatedByUserId == supervisorId && DbFunctions.TruncateTime(mu.CreatedOn)<=endDate && DbFunctions.TruncateTime(mu.CreatedOn)>=startDate).Select(u => new OrdersVM
                {
                    Status = u.Status,
                    ShippingCharges = u.ShippingCharges,
                    PaymentStatus = u.PaymentStatus,
        ...
        public ActionResult OrdersReport(DateTime? startDate, DateTime? endDate)
        {
            ...
            {
                List<OrdersVM> model = db.OrderHeader.Where(mu => mu.OrderByUserId == loginUser.Id && DbFunctions.TruncateTime(mu.CreatedOn) <= endDate && DbFunctions.TruncateTime(mu.CreatedOn) >= startDate).Select(u => new OrdersVM
                {
                    Status = u.Status,
                    ShippingCharges = u.ShippingCharges,
                    PaymentStatus = u.PaymentStatus,
